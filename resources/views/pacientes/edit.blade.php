<x-layouts.app :title="'Editar Paciente'">
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">Editar Paciente #{{ $paciente->id_paciente }}</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('pacientes.update', $paciente) }}" method="POST">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="date" name="fecha_nacimiento" class="form-control @error('fecha_nacimiento') is-invalid @enderror" value="{{ old('fecha_nacimiento', $paciente->fecha_nacimiento) }}">
                        @error('fecha_nacimiento')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Peso (kg)</label>
                        <input type="number" step="0.01" name="peso" class="form-control @error('peso') is-invalid @enderror" value="{{ old('peso', $paciente->peso) }}">
                        @error('peso')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Oxígeno (%)</label>
                        <input type="number" step="0.01" name="oxigeno" class="form-control @error('oxigeno') is-invalid @enderror" value="{{ old('oxigeno', $paciente->oxigeno) }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="id_servicio" class="form-select @error('id_servicio') is-invalid @enderror" required>
                            ...
                            @foreach($servicios as $servicio)
                                <option value="{{ $servicio->id_servicio }}" {{ old('id_servicio', $paciente->id_servicio) == $servicio->id_servicio ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="id_direccion" class="form-select @error('id_direccion') is-invalid @enderror">
                            ...
                            @foreach($direcciones as $direccion)
                                <option value="{{ $direccion->id_direccion }}" {{ old('id_direccion', $paciente->id_direccion) == $direccion->id_direccion ? 'selected' : '' }}>
